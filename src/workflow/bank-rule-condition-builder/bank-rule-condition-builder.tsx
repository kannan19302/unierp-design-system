...
/**
 * `<BankRuleConditionBuilder>` — Enterprise Bank Feed Condition Builder & Simulator.
 *
 * @maturity stable
 */
export const BankRuleConditionBuilder = forwardRef<
  HTMLElement,
  BankRuleConditionBuilderProps
>(
  (
    {
      initialRule,
    ...
    const [actionType, setActionType] = useState<
      "spend_money" | "receive_money" | "transfer"
    >(initialRule?.actionType || "spend_money");
    const [payee, setPayee] = useState<string>(
      initialRule?.payee || "Amazon Web Services Inc"
    );
    const [glAccount, setGlAccount] = useState<string>(
      initialRule?.glAccount || (glAccounts[0]?.code ?? "")
    );
    const [taxCode] = useState<string>(
      initialRule?.taxCode || "STANDARD-INPUT-20%"
    );
    ...
    const [simText, setSimText] = useState<string>(
      "POS DEBIT: Amazon Web Services AWS.AMZN.COM/BILL WA US $1,420.50"
    );
    ...
    const handleSimulate = () => {
      ...
      const passed =
        matchType === "all" ? results.every(Boolean) : results.some(Boolean);
      setSimResult(passed);
    };
        ...
        <div className={styles.formGrid}>
          ...
          <div className={styles.actionPanel}>
            <h3 className={styles.actionHeading}>
              THEN Automatically Generate &amp; Allocate:
            </h3>
            <div className={styles.actionFieldsRow}>
              <div className={styles.inputWrap}>
                <label
                  htmlFor={`${headingId}-action-type`}
                ...
                >
                  <option value="spend_money">
                    Spend Money (Vendor Payment)
                  </option>
                  <option value="receive_money">
                    Receive Money (Customer Receipt)
                  </option>
...
BankRuleConditionBuilder.displayName = "BankRuleConditionBuilder";
