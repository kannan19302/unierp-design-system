...
/**
 * `<CanaryRolloutProgressVisualizer>` — Enterprise Progressive Delivery & Automated Canary Analysis.
 *
 * @maturity stable
 */
export const CanaryRolloutProgressVisualizer = forwardRef<
  HTMLElement,
  CanaryRolloutProgressVisualizerProps
>(
  (
    {
    ...
    const remainingSeconds = Math.max(
      stepDurationSeconds - stepElapsedSeconds,
      0
    );
    ...
    const getStatusBadge = (st: CanaryStatus) => {
      switch (st) {
        case "analyzing":
          return (
            <span
              className={`${styles.statusBadge} ${styles.statusAnalyzing}`}
            >
              ● ACTIVE CANARY ANALYSIS
            </span>
          );
        case "paused":
          return (
    ...
    return (
      ...
      >
          ...
          <div className={styles.titleGroup}>
            ...
            <div>
              <div className={styles.metaRow}>
                ...
                <span className={styles.stableTag}>
        ...
        <div className={styles.trafficSection}>
          <div className={styles.trafficHeader}>
            <span className={styles.trafficLabel}>
              Live Traffic Weighting: <strong>{stablePercent}% Stable</strong>{" "}
              vs <strong>{currentTrafficPercent}% Canary</strong>
            </span>
            <span className={styles.stepTimer}>
              Step {currentStep} of {totalSteps}: {timeRemainingLabel} until
              auto-step promotion
            </span>
          </div>
          ...
          >
            <div
              className={styles.stableFill}
              style={{ inlineSize: `${stablePercent}%` }}
            >
              <span className={styles.barInnerText}>
                Stable ({stablePercent}%)
              </span>
            </div>
            <div
              className={styles.canaryFill}
              style={{ inlineSize: `${currentTrafficPercent}%` }}
            >
              <span className={styles.barInnerText}>
                Canary ({currentTrafficPercent}%)
              </span>
            </div>
          </div>
        </div>
        ...
        <div className={styles.metricsSection}>
          <h3 className={styles.metricsHeading}>
            Canary Telemetry Health Analysis
          </h3>
              ...
              return (
                ...
                >
                  ...
                  <div className={styles.thresholdInfo}>
                    <span>
                      Abort Threshold: &gt; {m.maxThreshold} {m.unit}
                    </span>
                  </div>
                </div>
              );
...
CanaryRolloutProgressVisualizer.displayName = "CanaryRolloutProgressVisualizer";
