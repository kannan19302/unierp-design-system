...
/**
 * `<ClinicalEdcFieldVerifier>` — Clinical Electronic Data Capture (EDC) Source Data Verification & Query Workbench.
 *
 * @maturity stable
 */
export const ClinicalEdcFieldVerifier = forwardRef<
  HTMLElement,
  ClinicalEdcFieldVerifierProps
>(
  (
    {
    ...
    const handleVerify = (fieldId: string) => {
      setFieldRecords((prev) =>
        prev.map((r) =>
          r.id === fieldId ? { ...r, sdvStatus: "VERIFIED" } : r
        )
      );
    ...
    const handleQuery = (fieldId: string) => {
      ...
      onRaiseQuery?.(
        fieldId,
        "Source documentation requested for discrepancy verification"
      );
    };
    ...
    const verifiedCount = fieldRecords.filter(
      (r) => r.sdvStatus === "VERIFIED"
    ).length;
    const queryCount = fieldRecords.filter(
      (r) => r.sdvStatus === "QUERY_OPEN"
    ).length;
    const pendingCount = fieldRecords.filter(
      (r) => r.sdvStatus === "PENDING_SDV"
    ).length;
      ...
      >
        <header className={styles.header}>
          <div className={styles.bannerRow}>
            <div className={styles.studyInfo}>
              <span className={styles.cfrBadge}>
          ...
          <div className={styles.titleRow}>
            ...
            <div className={styles.statsBar}>
              <span className={styles.statItem}>
                Verified:{" "}
                <strong className={styles.statValue}>{verifiedCount}</strong>
              </span>
              <span className={styles.statItem}>
                Pending SDV:{" "}
              ...
              <span className={styles.statItem}>
                Open Queries:{" "}
                <strong className={styles.statValue}>{queryCount}</strong>
              </span>
            </div>
          </div>
        </header>
        ...
        <div className={styles.tableWrapper}>
          <table
            ...
            aria-label="Clinical EDC Fields and SDV Audits"
          >
            <thead>
              <tr>
        ...
        <footer className={styles.footer}>
          <span>
            CDISC ODM / HL7 FHIR Clinical Research Data Standard Compliant.
          </span>
...
ClinicalEdcFieldVerifier.displayName = "ClinicalEdcFieldVerifier";
