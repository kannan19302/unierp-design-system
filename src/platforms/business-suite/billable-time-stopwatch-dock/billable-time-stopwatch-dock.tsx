...
/**
 * `<BillableTimeStopwatchDock>` — Enterprise Billable Time Capture & Stopwatch Dock.
 *
 * @maturity stable
 */
export const BillableTimeStopwatchDock = forwardRef<
  HTMLElement,
...
>(
  (
    ...
    const activeMatter =
      matters.find((m) => m.id === selectedMatterId) ?? matters[0] ?? null;
          ...
          <div className={styles.badgeRow}>
            ...
            <span
              className={`${styles.statusPill} ${
                isRunning ? styles.pillRunning : styles.pillStopped
              }`}
            >
              ...
              >
                {matters.map((m) => (
                  <option key={m.id} value={m.id}>
                    {m.clientName} — {m.matterCode}: {m.matterTitle} ($
                    {m.defaultHourlyRate}/hr)
                  </option>
                ))}
              </select>
            </div>
...
BillableTimeStopwatchDock.displayName = "BillableTimeStopwatchDock";
