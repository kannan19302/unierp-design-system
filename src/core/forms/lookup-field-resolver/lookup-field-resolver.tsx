...
export const LookupFieldResolver = React.forwardRef<
  HTMLDivElement,
  LookupFieldResolverProps
>((props, ref) => {
  ...
  return (
    <div ref={ref} className={styles.container} role="search" aria-label={label} style={{ position: 'relative' }}>
      ...
      {open && results.length > 0 && (
        <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 10, background: 'var(--color-surface-elevated)', border: '1px solid var(--color-border-default)', borderRadius: 'var(--radius-md)', boxShadow: 'var(--shadow-lg)', maxHeight: 200, overflowY: 'auto' }}>
          {results.map(r => (
            <button key={r.id} type="button" className={styles.item} style={{ width: '100%', border: 'none', cursor: 'pointer', background: 'none' }} onClick={() => { setQuery(r.label); setOpen(false); onSelect?.(r); }}>
...
LookupFieldResolver.displayName = "LookupFieldResolver";
