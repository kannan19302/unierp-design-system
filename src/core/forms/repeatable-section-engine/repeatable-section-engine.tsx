...
export const RepeatableSectionEngine = React.forwardRef<
  HTMLDivElement,
  RepeatableSectionEngineProps
>((props, ref) => {
  const { sectionLabel, fieldLabels, maxSections = 10 } = props;
  const [sections, setSections] = React.useState([0]);
  return (
    <div ref={ref} className={styles.container} role="group" aria-label={`Repeatable ${sectionLabel}`}>
        ...
        {sections.map((_, si) => (
          <div key={si} className={styles.section}>
            ...
            <div
              style={{
                display: "grid",
                gridTemplateColumns: `repeat(auto-fill, minmax(var(--space-48, 11.25rem), 1fr))`,
                gap: "var(--space-2)",
              }}
            >
              {fieldLabels.map((fl, fi) => (
                <div key={fi} className={styles.fieldGroup}>
                  <label className={styles.fieldLabel}>{fl}</label>
                  <input
                    className={styles.input}
                    placeholder={fl}
                    aria-label={`${sectionLabel} ${si + 1} ${fl}`}
                  />
                </div>
              ))}
            </div>
...
RepeatableSectionEngine.displayName = "RepeatableSectionEngine";
