...
/**
 * `<ClinicalTrialCohortRandomizer>` — Life Sciences & Clinical EDC Double-Blind Cohort Randomization Engine.
 *
 * @maturity stable
 */
export const ClinicalTrialCohortRandomizer = forwardRef<
  HTMLElement,
  ClinicalTrialCohortRandomizerProps
>(
  (
    {
      protocol,
    ...
    const [biomarkerStatus, setBiomarkerStatus] =
      useState<string>("HER2_POSITIVE");
    ...
    const totalEnrolled = protocol.activeArms.reduce(
      (sum, a) => sum + a.enrolledCount,
      0
    );
    ...
    const handleRandomize = (e: React.FormEvent) => {
      ...
      const chosenIndex = Math.floor(
        Math.random() * (protocol.activeArms.length || 1)
      );
      ...
      setLastRandomizedResult({
        subjectId: subjectId.trim(),
        armName:
          protocol.blinding === "DOUBLE_BLIND"
      ...
      >
        ...
        <form onSubmit={handleRandomize} className={styles.randomizationForm}>
          <h3 className={styles.sectionHeading}>
            Subject Stratification &amp; Enrollment Intake
          </h3>
        ...
        {lastRandomizedResult && (
          <div
            className={styles.dispensationBanner}
            role="region"
            aria-label="Randomization dispensation confirmation"
          >
            <div className={styles.dispensationHeader}>
              <span className={styles.dispensationBadge}>
                DISPENSATION CONFIRMATION
              </span>
              <span className={styles.tokenCode}>
                Token: {lastRandomizedResult.token}
              </span>
            </div>
            <div className={styles.dispensationGrid}>
              <div className={styles.dispItem}>
                ...
                <span className={styles.dispValue}>
                  {lastRandomizedResult.subjectId}
                </span>
              </div>
              <div className={styles.dispItem}>
                <span className={styles.dispLabel}>Assigned Cohort:</span>
                <span className={styles.dispValue}>
                  {lastRandomizedResult.armName}
                </span>
              </div>
              <div className={styles.dispItem}>
                <span className={styles.dispLabel}>Allocated Kit Barcode:</span>
                <code className={styles.barcodeValue}>
                  {lastRandomizedResult.kitBarcode}
                </code>
              </div>
            </div>
          </div>
...
ClinicalTrialCohortRandomizer.displayName = "ClinicalTrialCohortRandomizer";
