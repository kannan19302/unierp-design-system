...
describe("ComboBox", () => {
  it("renders trigger with placeholder", () => {
    ...
    expect(
      screen.getByRole("button", { name: "Select ERP..." }),
    ).toBeInTheDocument();
  });
  ...
  it("selects option on click in single select mode", async () => {
    ...
    await userEvent.click(
      screen.getByRole("option", { name: "Oracle NetSuite" }),
    );
  ...
  it("filters options by search term", async () => {
    ...
    expect(
      screen.queryByRole("option", { name: "SAP S/4HANA" }),
    ).not.toBeInTheDocument();
  });

  it("has no axe violations", async () => {
    const { container } = render(
      <ComboBox options={OPTIONS} placeholder="Select ERP..." />,
    );
