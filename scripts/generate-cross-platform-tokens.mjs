...
const JSON_FILE = join(TOKENS, "tokens.g.json");
const DIST_TOKENS_DIR = join(HERE, "..", "dist", "tokens");
const DIST_JSON_FILE = join(DIST_TOKENS_DIR, "tokens.g.json");
...
function emitScale(theme = "strata") {
  const t = readTheme(theme);
  const rows = [];
 ...
 * `--accent`, …) and repoints them at Strata values, so the 100-line page
...
function emitDesktopCss() {
  const light = readTheme("strata");
  const dark = readTheme("strata-dark");
  ...
  return [
    ...
    " */",
    ':root, [data-theme="strata"], [data-theme="meridian"] {',
    rows(light),
    "  --accent-grad: var(--accent);",
    "}",
    "",
    '[data-theme="dark"], [data-theme="strata-dark"], [data-theme="meridian-dark"] {',
    rows(dark),
    "  --accent-grad: var(--accent);",
    "}",
    "",
    "@media (prefers-color-scheme: dark) {",
    '  :root:not([data-theme="light"]):not([data-theme="strata"]):not([data-theme="meridian"]) {',
    ...
    JSON.stringify(
      {
        ...
        scale: {
          spacing: Object.fromEntries(
            Object.entries(SPACING).map(([k, v]) => [k, readTheme("strata")[v] ?? null])
          ),
          radius: Object.fromEntries(
            Object.entries(RADIUS).map(([k, v]) => [k, readTheme("strata")[v] ?? null])
          ),
          typography: Object.fromEntries(
            Object.entries(TEXT).map(([k, v]) => [k, readTheme("strata")[v] ?? null])
          ),
          touch: { minTouchTarget: "44px" },
        },
        density: {
          "ultra-compact": "24px",
          compact: "28px",
...
${emitTheme("UniTokens", "strata")}

${emitTheme("UniTokensDark", "strata-dark")}
...
if (CHECK_ONLY) {
  ...
  const currentJson = existsSync(JSON_FILE) ? readFileSync(JSON_FILE, "utf8") : "";
  if (currentJson !== json) {
    console.error(
      "  FAIL  JSON token dictionary has drifted from the design system.\n" +
        "        Run: node scripts/generate-cross-platform-tokens.mjs",
    );
    process.exit(1);
  }
